...
_FACILITY_CONSOLE_ASSET_DIR = Path(__file__).resolve().parent / "assets"
_FACILITY_CONSOLE_APP_JS_PATH = _FACILITY_CONSOLE_ASSET_DIR / "facility_console_app.js"


@lru_cache(maxsize=1)
def _facility_console_script_text() -> str:
    return _FACILITY_CONSOLE_APP_JS_PATH.read_text(encoding="utf-8")


@lru_cache(maxsize=1)
def facility_console_script_version() -> str:
    payload = _facility_console_script_text().encode("utf-8")
    return hashlib.sha256(payload).hexdigest()[:12]


def facility_console_script_text() -> str:
    return _facility_console_script_text()


def facility_console_script_url() -> str:
    return f"/web/console/app.js?v={facility_console_script_version()}"
...
"""
    ...
    rendered = rendered.replace("__SCRIPT_URL__", facility_console_script_url())
    return rendered
