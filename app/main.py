...
def _write_alert_delivery(
    *,
    ...
    return _ops_record_service_module()._write_alert_delivery(
...
def _normalize_mttr_slo_recover_state(value: str | None) -> str:
    return _ops_alert_service_module()._normalize_mttr_slo_recover_state(value)


def _default_mttr_slo_policy() -> dict[str, Any]:
    return _ops_alert_service_module()._default_mttr_slo_policy()


def _legacy_mttr_slo_policy() -> dict[str, Any]:
    return _ops_alert_service_module()._legacy_mttr_slo_policy()
...
def _parse_mttr_slo_policy_json(raw: Any) -> dict[str, Any]:
    return _ops_alert_service_module()._parse_mttr_slo_policy_json(raw)
...
def _upsert_mttr_slo_policy(payload: dict[str, Any]) -> tuple[dict[str, Any], datetime, str]:
    return _ops_alert_service_module()._upsert_mttr_slo_policy(payload)
...
def _build_alert_webhook_request_headers() -> dict[str, str]:
    return _ops_alert_service_module()._build_alert_webhook_request_headers()


def _truncate_alert_text(value: Any, max_length: int = 240) -> str:
    return _ops_alert_service_module()._truncate_alert_text(value, max_length=max_length)


def _humanize_alert_event_type(event_type: str) -> str:
    return _ops_alert_service_module()._humanize_alert_event_type(event_type)


def _build_alert_message_summary(event_type: str, payload: dict[str, Any]) -> dict[str, Any]:
    return _ops_alert_service_module()._build_alert_message_summary(event_type, payload)
...
def _render_alert_payload_for_target(
    *,
    ...
    return _ops_alert_service_module()._render_alert_payload_for_target(
        event_type=event_type,
        payload=payload,
...
def _build_alert_delivery_record_payload(
    *,
    ...
    return _ops_alert_service_module()._build_alert_delivery_record_payload(
        payload=payload,
        target_kind=target_kind,
    )
...
def _is_alert_failure_status(status: str) -> bool:
    return _ops_alert_service_module()._is_alert_failure_status(status)
...
def _compute_alert_channel_guard_state(
    ...
    *,
    ...
    return _ops_alert_service_module()._compute_alert_channel_guard_state(
        target,
        now=now,
        event_type=event_type,
...
def _build_alert_delivery_archive_csv(rows: list[dict[str, Any]]) -> str:
    return _ops_alert_service_module()._build_alert_delivery_archive_csv(rows)
