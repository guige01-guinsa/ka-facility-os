"""add adoption w14 execution tracker tables

Revision ID: 20260302_0020
...
def upgrade() -> None:
    ...
    if not _has_table(inspector, "adoption_w14_tracker_items"):
        op.create_table(
    ...
    inspector = sa.inspect(bind)
    if not _index_exists(inspector, "adoption_w14_tracker_items", "ix_w14_tracker_site_status"):
        op.create_index(
            "ix_w14_tracker_site_status",
            "adoption_w14_tracker_items",
            ["site", "status"],
            unique=False,
        )
    if not _index_exists(inspector, "adoption_w14_tracker_items", "ux_w14_tracker_site_type_key"):
        op.create_index(
            "ux_w14_tracker_site_type_key",
            "adoption_w14_tracker_items",
            ["site", "item_type", "item_key"],
            unique=True,
        )
    if not _index_exists(inspector, "adoption_w14_tracker_items", "ix_w14_tracker_assignee"):
        op.create_index(
            "ix_w14_tracker_assignee",
            "adoption_w14_tracker_items",
            ["assignee"],
            unique=False,
        )
    ...
    inspector = sa.inspect(bind)
    if not _has_table(inspector, "adoption_w14_evidence_files"):
        op.create_table(
    ...
    inspector = sa.inspect(bind)
    if not _index_exists(inspector, "adoption_w14_evidence_files", "ix_w14_evidence_tracker_uploaded_at"):
        op.create_index(
            "ix_w14_evidence_tracker_uploaded_at",
            "adoption_w14_evidence_files",
            ["tracker_item_id", "uploaded_at"],
            unique=False,
        )
    if not _index_exists(inspector, "adoption_w14_evidence_files", "ix_w14_evidence_site_uploaded_at"):
        op.create_index(
            "ix_w14_evidence_site_uploaded_at",
            "adoption_w14_evidence_files",
            ["site", "uploaded_at"],
            unique=False,
        )
    if not _index_exists(inspector, "adoption_w14_evidence_files", "ix_w14_evidence_sha256"):
        op.create_index(
            "ix_w14_evidence_sha256",
            "adoption_w14_evidence_files",
            ["sha256"],
            unique=False,
        )
    if not _index_exists(inspector, "adoption_w14_evidence_files", "ix_w14_evidence_storage_key"):
        op.create_index(
            "ix_w14_evidence_storage_key",
            "adoption_w14_evidence_files",
            ["storage_key"],
            unique=False,
        )
    ...
    inspector = sa.inspect(bind)
    if not _has_table(inspector, "adoption_w14_site_runs"):
        op.create_table(
    ...
    inspector = sa.inspect(bind)
    if not _index_exists(inspector, "adoption_w14_site_runs", "ix_w14_site_runs_status"):
        op.create_index(
            "ix_w14_site_runs_status",
            "adoption_w14_site_runs",
            ["status"],
            unique=False,
        )
    if not _index_exists(inspector, "adoption_w14_site_runs", "ux_w14_site_runs_site"):
        op.create_index(
            "ux_w14_site_runs_site",
            "adoption_w14_site_runs",
            ["site"],
            unique=True,
        )
