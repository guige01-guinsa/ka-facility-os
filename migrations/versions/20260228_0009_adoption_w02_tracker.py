"""add adoption w02 execution tracker tables
...
def upgrade() -> None:
    ...
    if not _has_table(inspector, "adoption_w02_tracker_items"):
        op.create_table(
    ...
    inspector = sa.inspect(bind)
    if not _index_exists(inspector, "adoption_w02_tracker_items", "ix_w02_tracker_site_status"):
        op.create_index(
            "ix_w02_tracker_site_status",
            "adoption_w02_tracker_items",
            ["site", "status"],
            unique=False,
        )
    if not _index_exists(inspector, "adoption_w02_tracker_items", "ux_w02_tracker_site_type_key"):
        op.create_index(
            "ux_w02_tracker_site_type_key",
            "adoption_w02_tracker_items",
            ["site", "item_type", "item_key"],
            unique=True,
        )
    if not _index_exists(inspector, "adoption_w02_tracker_items", "ix_w02_tracker_assignee"):
        op.create_index(
            "ix_w02_tracker_assignee",
            "adoption_w02_tracker_items",
            ["assignee"],
            unique=False,
        )
    ...
    inspector = sa.inspect(bind)
    if not _has_table(inspector, "adoption_w02_evidence_files"):
        op.create_table(
    ...
    inspector = sa.inspect(bind)
    if not _index_exists(inspector, "adoption_w02_evidence_files", "ix_w02_evidence_tracker_uploaded_at"):
        op.create_index(
            "ix_w02_evidence_tracker_uploaded_at",
            "adoption_w02_evidence_files",
            ["tracker_item_id", "uploaded_at"],
            unique=False,
        )
    if not _index_exists(inspector, "adoption_w02_evidence_files", "ix_w02_evidence_site_uploaded_at"):
        op.create_index(
            "ix_w02_evidence_site_uploaded_at",
            "adoption_w02_evidence_files",
            ["site", "uploaded_at"],
            unique=False,
        )
